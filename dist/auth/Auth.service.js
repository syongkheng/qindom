...
class AuthService {
    ...
    login(_a) {
        return __awaiter(this, arguments, void 0, function* ({ username, password, system, }) {
            ...
            LoggingUtilities_1.LoggingUtilities.service.debug("AuthService.login", `Password comparison result: ${isValidPassword}`);
            if (!isValidPassword) {
                throw new AppExceptions_1.Exceptions.InvalidLoginCredentials();
            }
            ...
            LoggingUtilities_1.LoggingUtilities.service.info("AuthService.login", `Success login for ${username}_${system} with token: ${generatedToken.substring(0, 10) + "..."}`);
