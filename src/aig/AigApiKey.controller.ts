...
import { AigApiKeyService } from "./AigApiKey.service.js";

export default function createAigApiKeyController(db: KnexSqlUtilities) {
  const router = Router();
  const service = new AigApiKeyService(db);
  ...
  router.get("/api-key", async (req: RequestWithUserInfo, res: Response) => {
    ...
    try {
      ...
      const logEvent = logContext
        ? LoggingUtilities.request.branch(logContext, "SERVICE", "Get API key status")
        : undefined;
      const result = await service.getKeyStatus(user.id, logEvent);
      return cr.ok(result);
    } catch (err) {
      return handleException(err, cr, "AigApiKeyController.GET /api-key", "Failed to get API key status");
    }
  });
  ...
  router.post("/api-key", async (req: RequestWithUserInfo, res: Response) => {
    ...
    try {
      ...
      return handleException(err, cr, "AigApiKeyController.POST /api-key", "Failed to generate API key");
    }
  });
  ...
  router.delete("/api-key", async (req: RequestWithUserInfo, res: Response) => {
    ...
    try {
      ...
      return handleException(err, cr, "AigApiKeyController.DELETE /api-key", "Failed to revoke API key");
    }
  });
