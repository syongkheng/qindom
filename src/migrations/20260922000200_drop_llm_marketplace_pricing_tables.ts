...
// Marketplace Pricing admin feature removed — it was scaffolding for an AI
// marketplace (wallet/chat/api-key billing) whose backend was never actually
// implemented. tb_llm_api_key was unused (no service/controller ever queried
// it). tb_llm_model is untouched — it's still used by the unrelated, live
// /v1/llm echo API (src/llm/Llm.v1.*).
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`DROP TABLE IF EXISTS tb_llm_api_key`);
}
